...
const SolutionCard = ({ title, description, link, image }) => {
  return (
    <div className="solution-card h-100 position-relative overflow-hidden rounded-4 hover-lift" style={{ minHeight: '350px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
      <img src={image} alt={title} className="w-100 h-100 object-fit-cover position-absolute top-0 start-0" style={{ transition: 'transform 0.5s ease' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'} />
